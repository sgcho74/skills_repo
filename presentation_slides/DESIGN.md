...
All UI icons across HTML slides and presentation assets use **[Lucide](https://lucide.dev/)** — a clean, consistent open-source SVG icon set.

#### CDN Integration

```html
<!-- Place in <head> before </head> -->
...
```html
<!-- Declare icon via data attribute -->
<i data-lucide="search"></i>

<!-- Initialize in script (after DOM ready) -->
<script>
  lucide.createIcons();
</script>
```
...
| Context | Size | CSS |
|---|---|---|
| Hero card icon | 44×44px | `.card-icon svg { width: 44px; height: 44px; }` |
| Feature / grid card icon | 36×36px | `.feature-icon svg { width: 36px; height: 36px; }` |
| Diagram node icon | 28×28px | `.node-icon svg, .arch-box-icon svg { width: 28px; height: 28px; }` |
| Core diagram center | 34×34px | `.core-icon svg { width: 34px; height: 34px; }` |
| Summary / closing card | 36×36px | `.summary-icon svg { width: 36px; height: 36px; }` |
| Step / process icon | 28×28px | `.step-icon svg { width: 28px; height: 28px; }` |

**Universal rule:** `stroke-width: 1.5` on all icons. Lucide's default of 2 reads too heavy against the dark presentation background.

#### Icon Vocabulary (Standard Map)

| Semantic | Lucide Name |
|---|---|
| 검색 / Use Case 도출 | `search` |
| AI Agent / 챗봇 | `bot` |
| 설정 / BPMS | `settings-2` |
| 통계 / 분석 | `bar-chart-2` |
| 경고 / 주의사항 | `alert-triangle` |
| 트렌드 / 추천 | `trending-up` |
| 문서 자동생성 | `file-text` |
| 지식 검색 | `book-open` |
| 보고서 / 클립보드 | `clipboard-list` |
| 건설 / 현장 | `building-2` |
| DB / Oracle | `database` |
| 그래프 DB / 네트워크 | `network` |
| 복합 라우팅 | `shuffle` |
| 채팅 / 자연어 | `message-square` |
| 공장 / 생산라인 | `factory` |
| 자동화 / 에너지 | `zap` |
| 네비게이션 / 워크플로우 | `map` |
| 목표 / 타당성 | `target` |
...
1. **Never mix emoji and Lucide icons** in the same component layer — choose one per slide.
2. **Color via `currentColor`** — Lucide SVGs inherit the parent element's `color` CSS property, so icon tinting is handled by setting color on the wrapper element, not the SVG directly.
3. **No fill** — All icons use `fill="none"` (Lucide default). Do not apply fill to icons.
4. **Accessibility** — Add `aria-label` or `aria-hidden="true"` depending on context. Decorative icons should use `aria-hidden="true"`.
